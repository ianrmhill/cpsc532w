...
def interpret(ast, sigma, env):
    ...
    if ast[0] == 'let':
        val, sig, e = interpret(ast[1][1], sigma, env)
        env.add(ast[1][0], val)
    ...
    # Case where we are observing a random variable
        ...
        dist, sig1, e1 = interpret(ast[1], sigma, env)
        obs, sig2, e2 = interpret(ast[2], sig1, e1)
        sig2['logw'] = sig2['logw'] + dist.log_prob(obs)
        return obs, sig2, e2

    # Case 'if' where expression is the ternary
    elif ast[0] == 'if':
        truth, sig, e = interpret(ast[1], sigma, env)
        if truth:
            return interpret(ast[2], sig, e)
        else:
            return interpret(ast[3], sig, e)
